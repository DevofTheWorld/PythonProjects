...
def main():
    ...
    for item in path.iterdir():
        if item.is_file():
            if item.name.endswith((".docx", ".pdf", ".xlsx", ".pptx", ".csv", ".gz")):
                item.move(
                    Path("/Users/anthonylumantao/Downloads/Documents") / item.name
                )
            ...
            elif item.name.startswith(("Screenshots")) or item.name.endswith(
                (".gif", ".png", ".jpg", ".jpeg")
            ):
                item.move(Path("/Users/anthonylumantao/Downloads/Images") / item.name)
            elif item.name.endswith(
                (".drawio", ".aep", ".pkt", ".zip", "md", ".piskel")
            ):
                item.move(
                    Path("/Users/anthonylumantao/Downloads/Activities") / item.name
                )
            else:
                item.unlink()
